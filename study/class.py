...
# 4. 또한 class로 만든 object는 인스턴스라고도 한다. 만약 cookie()라는 class로 a라는 객체를 만든다면 a는 객체이면서 cookie()의 인스턴스이기도 하다.
...
# 7. class를 만들때에는 무작정 만드는 것보다 미리 이 class가 만든 객체가 어떤 식으로 동작하게 될 것인지를 미리 구상한 후에 구상한 기능들을 하나씩 해결하면서 만드는 것이 좋다.
...
# 위의 object a와 b는 서로 영향을 주지 않고 고유의 성향을 가지므로 object a에서 first의 값에 4를 주고, object b에서 first의 값에 3을 줘도 a나 b의 값이 변경되거나 하지 않는다.
# 객체끼리는 서로 완전히 독립적인 존재이다.
# 이는 변수의 주소값을 확인할 수 있는 id() 함수를 통해 쉽게 확인해볼 수 있는데, id(a.first) id(b.first)의 주소값은 다르다는 것을 확인해볼 수 있다.

# 위의 FourCal() class의 a 인스턴스에 setdata 메소드를 수행하지 않고 바로 a 인스턴스에 add 메소드를 수행하면 a 인스턴스의 변수가 생성되지 않기 때문에 오류가 난다.
...
# class의 상속(inheritance) : class에서도 상속의 개념이 가능하다. 어떤 class를 만들 때 다른 class의 기능을 물려받을 수 있게 만드는 것이 가능하다는 뜻이다.
# 보통 상속은 기존 class를 변경하지 않고 기능을 추가하거나 기존 기능을 변경하려고 할 때 사용한다.
# 물론 개인이 작성한 class라면 class를 직접 수정할수도 있겠지만, 라이브러리나 프레임워크의 class를 사용할 때 수정이 허용되지 않는 상황이 있기 때문에, 이럴때는 상속을 사용해야 한다.
