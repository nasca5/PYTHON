...
# 집합 자료형은 중복을 허용하지 않고, 순서가 없다는 특징을 지닌다. 따라서 set 자료형은 인덱싱으로 값을 얻을 수 없다.
# 인덱싱으로 접근하고 싶다면 리스트나, 튜플로 변환한 다음 하면 된다. 
# set 자료형은 교집합, 합집합, 차집합을 구할 때 주로 쓰인다.(&, |, -)

# set 자료형 관련 함수
# 1. 값 1개 추가하기 -> x.add(n) x라는 set 자료형에 n이라는 값을 1개 추가할 수 있다.
# 2. 값 여러 개 추가하기 -> x.update([...]) x라는 set 자료형에 [...]형식으로 여러 개의 값을 추가할 수 있다.
# 3. 값을 제거할 때 -> x.remove(n) x라는 set 자료형에서 n이라는 값을 제거할 수 있다.


##########################################################################################################################################################################################################################################

# 불 자료형(bool)
# 참과 거짓을 나타내는 자료형이다. 불 자료형은 참(True)와 거짓(False)만 가질 수 있다.
# 자료형에도 참과 거짓이 있는데, ""의 빈 문자열과 리스트, 튜플, 딕셔너리 등이 비어있으면 이것 또한 거짓이다. None도 거짓이다. 그 외에 나머지는 모두 참의 값을 가진다.
# 불 연산 : bool(n)에서 ()안의 n의 값이 참인지 거짓인지에 따라 True와 False를 출력한다.

##########################################################################################################################################################################################################################################

# 자료형의 값을 저장하는 공간, 변수
# 파이썬에서는 변수에 저장된 값을 스스로 판단하여 자료형을 지정하기 때문에 따로 자료형을 지정해줄 필요가 없어 편리하다.
# 파이썬에서 사용하는 변수는 객체를 가리키는 것이라고도 말할 수 있다.
# 변수에 list와 같은 객체들을 선언하는 것은 실제 변수에 리스트가 할당된 것이 아닌 리스트의 주소를 참조하는 것이다.
# 따라서 a = [1, 2, 3]인 상태에서 b = a라고 한다면 a와 b는 동일하다고 볼 수 있다. 단지 리스트 [1, 2, 3]을 참조하는 변수가 a 하나에서 b까지 둘로 늘어난 것 뿐이다.
# 따라서 만약 a list의 요소 값을 바꾼다면 자연히 b도 똑같이 바뀌게 된다. 이것을 방지하고 싶다면, [:]처럼 슬라이싱을 사용해 처음부터 끝까지 슬라이싱하여 가져오거나, copy 모듈을 사용하면 된다.
# 